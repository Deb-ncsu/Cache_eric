...
int main (int argc, char* argv[])
{
    ...
    char str[2];
    Cache my_cache(params.l1_size , params.block_size , params.l1_assoc, params.l2_size, params.block_size , params.l2_assoc ,1, params.vc_num_blocks,0);
    int count = 1;
    while(fscanf(FP, "%s %lx", str, &addr) != EOF)
    {
        ...
        rw = str[0];
        if (rw == 'r') {
            //printf("# %d: %s %lx\n", count, "read", addr);           // Print and test if file is read correctly
            my_cache.read_data(addr);
	    //cout << "----------------------------------------" << endl;
	} else if (rw == 'w') {
            //printf("# %d: %s %lx\n", count, "write", addr);          // Print and test if file is read correctly
	    my_cache.write_data(addr);
	    //cout << "----------------------------------------" << endl;
	}
	count++;
        /*************************************
